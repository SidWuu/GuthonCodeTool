...
- 从谷神源码表拉取页面和过程函数源码。
- 支持相互隔离的产品层、项目层和项目快照 effective 视图。
...
- 生成产品源码索引。
- 生成仅包含对应项目快照的 effective 源码索引，不混入当前产品源码。
- 生成静态调用索引。
- 记录低置信动态调用点。
- 全量 Markdown 索引输出到 `var/knowledge` 供人工浏览；AI 助手通过 SQLite 局部上下文查询定位源码和调用链。
...
var/source/effective/   项目自身快照的规范化只读视图
...
`ACTIVE` 同时决定源码同步范围，以及表结构、单据类型默认连接的产品库或项目库。`table_schema_data_source_ids` 控制未显式传入数据源编码时的 SQL 导出范围。
...
状态包括 `CLEAN`、`LOCAL_CHANGED`、`UPSTREAM_CHANGED`、`CONFLICT` 和 `UPSTREAM_MISSING`。`CONFLICT` 时手动拉取会保留工作副本并返回失败提示。
