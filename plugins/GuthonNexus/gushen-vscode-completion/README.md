...
2. Run `Guthon Nexus: 添加产品或项目` on first use and whenever development adds another project. Products and projects are peer workspaces: a project is an independent exported version snapshot, so the wizard creates it directly without selecting another workspace. The wizard ends immediately after SVN or DATABASE is selected and creates an initially unconfigured Nexus. For SVN, expand that Nexus and use `设置工作区 SVN 登录`, `导入/粘贴 SVN checkout 配置`, and `编辑 SVN 范围配置`; multiline paste and explicitly selected `svnCheckoutHere.sh/.bat` are supported. Multiple checkout URLs for one product are normalized into one shared `svn.url` plus category entries in `svn.scope`, even before system mappings are configured. DATABASE datasource settings are added later. Do not add a `source_mode` field. Legacy exact entries and `checkoutPaths` remain compatible; imported scripts are parsed but never executed.
3. Expand `项目` and choose a `PRD` or `PRJ` workspace.
4. For DATABASE, configure its datasource and run the selected workspace's full synchronization command. For SVN, use `导入/粘贴 SVN checkout 配置`, review the YAML, then run `从 SVN 范围配置检出/更新`.
...
- Imports a workspace's SVN scope from a selected `.sh/.bat` file or pasted checkout text into one shared `svn.url` plus every unique relative checkout path in `svn.scope`, preserving unrelated YAML comments and entries. With no system mappings all exact paths are checked out; configured mappings filter that upper bound. The expanded exact manifest is stored in workspace `context/authorized-scope.json` for program use, while legacy `checkoutPaths` remains supported.
