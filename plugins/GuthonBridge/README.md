...
Guthon Bridge 是面向 Guthon 在线开发页面的本地辅助扩展，由 Chrome 扩展和本地 HTTP 桥接服务组成。可搭配 Guthon Hub 功能使用。
...
- 页面拉取：在过程函数页识别当前脚本，并保存为本地文件。
- 函数定位：在过程函数脚本中，点击谷神自带的定位按钮，高亮过程函数名时，滚动条拉取到对应的过程函数
**源码拉取**
- 通过本地 HTTP 服务保存文件，并记录远端对象、本地路径和版本元数据。
- 在模块开发页/过程函数页调用 Guthon Hub，从源码表拉取已存入源码并生成工作副本。
