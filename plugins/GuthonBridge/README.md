...
以上源码拉取和数据库资料导出只适用于工作区摘要为 `sourceMode=database` 的项目。Bridge 会先读取工作区摘要；SVN 工作区隐藏页面悬浮按钮和弹窗中的旧拉取入口，旧版扩展直接调用这些接口时，服务端也会返回明确的模式错误。SVN 初始化、刷新、扫描和虚拟写回统一从 Guthon Nexus 操作。
