...
3. `Guthon Nexus: 初始化源码索引`.
4. `Guthon Nexus: 同步当前 ACTIVE 源码`.
...
`Guthon Nexus: 同步当前 ACTIVE 全部资料` additionally exports table schemas, bill types, system scripts, and views. All command output is shown in the `GuthonCodeTool` output channel.
...
- Initializes, synchronizes, and reindexes the current `sync.ACTIVE` source.
