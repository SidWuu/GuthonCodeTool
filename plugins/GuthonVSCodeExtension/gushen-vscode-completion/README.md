...
2. Fill the generated `<本地数据目录>/config/*.yaml`; do not add a `source_mode` field. Each product/project defaults to DATABASE and has its own `源码来源：DATABASE/SVN` action. For SVN, maintain the matching `products.yaml` or `projects.yaml` item's compact `svn.url`/`svn.scope`; `systems` and `datasources` are expanded from `systems.include.mappings`, and an omitted scope means mapped systems/datasources only. Legacy exact entries and `checkoutPaths` remain compatible. The `导入 SVN checkout 配置` action accepts a downloaded `svnCheckoutHere.sh/.bat` or pasted checkout text, writes the shared root/categories into the existing YAML, and never executes the script. Both script formats remain compatible as one-time import inputs.
