...
2. Run `Guthon Nexus: 添加产品或项目` on first use and whenever development adds another project. Products and projects are peer workspaces: a project is an independent exported version snapshot, so the wizard creates it directly without selecting another workspace. DATABASE setup asks only for a MySQL/MariaDB/PostgreSQL URL and one combined username/password input, then derives the remaining datasource fields. The first SVN item records the shared username. After generation it asks whether to open the matching YAML so the user can confirm the system alias, system ID, and data-source ID. For SVN it can continue directly to checkout-config import and initialization. Do not add a `source_mode` field. Legacy exact entries and `checkoutPaths` remain compatible; imported scripts are parsed but never executed.
3. Expand `项目` and choose a `PRD` or `PRJ` workspace.
4. For DATABASE, run the selected workspace's full synchronization command. For SVN, use `导入 SVN checkout 配置` when starting from a script/paste, review the YAML, then run `从 SVN 范围配置检出/更新`.
...
Database workspace nodes retain their existing source/metadata synchronization and Workcopy flow. Nexus lists DATABASE and SVN workspaces together; each node changes only its own provider and immediately refreshes the tree. SVN nodes use a reviewed exact-URL manifest, aggregate multiple physical working copies, expose a business source tree and virtual documents that write directly back to the checkout, and register one source-control provider per workspace. `systems.include.mappings` uses each system alias as a key with one `system_id` and one `data_source_id`; Nexus intersects those IDs with the checkout-script scope. Missing or invalid mappings block checkout. The cached Chinese system metadata is used only to label SCM datasource groups and never expands SVN authorization. Every repository in one script reuses the same workspace username and the SVN client's system credential cache. `Ctrl+S` is local-only. SCM changes are grouped by datasource subsystem: one group owns its datasource working copy and every mapped system working copy, so `0008` is shown as `贸易系统` and includes domestic and international trade sources. The workspace provider retains all-submit and all-update actions. PAGE changes open a readable script/SQL/field projection by default, while the context menu retains the exact raw JSON diff. `管理本地源码变更` lists Nexus and external modifications and supports multi-select save/revert. Nexus-managed rows can also be multi-selected and submitted directly; the existing exact-file commit path is reused instead of adding a Git staging area. Text-conflict rows open VS Code's three-way merge editor with SVN's previous base, local, incoming, and physical checkout result files, then require an explicit mark-resolved action. `保存到谷神` is always available in Nexus SVN mode; the SCM commit message is optional, and selections spanning physical working copies are committed in groups without claiming that the later Guthon platform submission is complete. Status decorations distinguish added, deleted, modified, and conflicted files.
...
- Imports a workspace's SVN scope from a selected `.sh/.bat` file or pasted checkout text into compact `svn.url`/`svn.scope` in the existing `products.yaml`/`projects.yaml`, preserving unrelated YAML comments and entries; the expanded exact manifest is stored in workspace `context/authorized-scope.json` for program use, while legacy `checkoutPaths` remains supported.
...
- Adds native “谷神源码” toolbar actions for locating, jumping, expanding or collapsing the selected node, opening VS Code's directly visible tree find box, and refreshing. A procedure node's context menu can copy its function name or qualified `package.function` identity, and list indexed callers for exact-line navigation. Find uses cached nodes with filter + fuzzy defaults, while long source labels use the native horizontal scrollbar (`workbench.list.horizontalScrolling`, user-overridable).
