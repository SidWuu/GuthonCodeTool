...
打开 VS Code 左侧“源代码管理”（分支图标），即可看到当前工作副本的“工作副本更改”。新版会按实际 checkout 显示中文存储库，例如“国际贸易 · 页面”“贸易系统 · 过程函数”“风险管理 · 系统脚本”；变更文件优先使用当前项目 AI 索引显示页面、过程函数、服务组件、系统脚本、数据表和视图的中文名称，悬浮提示仍保留原始 SVN 路径和编码。默认点击文件打开“脚本/SQL 可读差异”：页面 JSON 只展开事件脚本和数据源 SQL，便于看业务修改。右键选择“查看真实文件差异”可核对 SVN `BASE` 与当前原始文件的完整文本；两种查看方式都不会改动源码。文件被删除或本地缺失时，差异左侧仍读取 SVN `BASE`，右侧为空，因此能看到整份删除内容。
