...
- 变更列表中的主页面现在包含所属功能模块，例如“策略方案 · 主页面”，避免多个系统中的主页面难以区分。

## 0.7.22

- 修复分片 SVN 初始化或更新遇到网关超时后留下 `incomplete`、`wc-locked` 工作副本的问题。
- 初始化和更新前自动检查工作副本健康状态；发现 SVN 管理锁时先安全执行 `svn cleanup`，再继续 `svn update`。
- Cleanup 只清理 SVN 管理锁，不删除本地文件、未纳入版本控制的文件或用户修改。
- 502/Bad Gateway 失败时保留已下载内容，并提示稍后重试以继续恢复。

## 0.7.21

- 仅支持新式 SVN 项目结构：systems/* 和 datasources/*。
- 支持完整项目工作副本与新式分片工作副本的自动识别。
- 页面、系统脚本、过程函数、表和视图统一使用新的真实源码路径。
- 从 $.中文名 和各目录下的 index.md 读取系统、数据源及对象显示名称。
- AI 索引升级为 systems-datasources 布局，旧索引不再作为有效索引使用。
- 配置模板只保留顶层 username、项目地址和可选 checkout_paths。

## 0.7.20

- 修复更新前备份中文 GSS/Markdown 文件时因 `svn diff` 本地编码转换失败（`E000022`）而取消更新的问题。
- 更新前改为直接复制工作区文件字节并生成 `manifest.json`，不依赖 SVN diff 输出编码；删除/缺失状态也会记录在快照清单中。

## 0.7.19

- 兼容页面数据源的两种运行类型：`dsType=1` 按 `script` 字段识别为 GSS 数据源，其他类型继续按 `sql` 字段识别为 SQL 数据源。
- 数据源脚本在中文目录中显示为“数据源脚本（GSS）”，虚拟文档使用 GSS 语法高亮、格式化和回写路径；SQL 数据源行为保持不变。
- AI 页面索引和可读差异分别列出数据源 GSS 与数据源 SQL，避免把数据源脚本误归类为普通页面事件脚本。

## 0.7.18

- 支持过程函数和页面服务组件的 `X.gss` / `X.inherit.gss` 同目录继承配对，忽略注释与字符串中的伪 `@inherit()`。
- 中文源码目录将继承子级与父级实现并列展示；父级使用锁定/只读标记，不计入业务对象数量，异常继承显示警告原因。
- `.inherit.gss` 以只读虚拟文档打开；子文件中的 `@inherit()` 支持 macOS Command+点击和 Windows Ctrl+点击跳转父级实现。
- 新增“展开继承到子文件”命令：确认后将父级业务脚本原位写入子文件，保留子级头注释和附加代码，父文件保持不变且操作可撤销。
- AI 索引升级为版本 2：父文件不再生成重复业务对象，子对象增加 `inheritance`、`effectiveSourcePaths` 和 `inherits` 关系，并从父子有效源码共同提取调用与 SQL 依赖。
- AI 索引清单增加继承中、已覆盖和继承异常数量；复制 AI 上下文会同时说明子文件、只读父级路径及继承状态。

## 0.7.17

- 支持两种项目 SVN 结构：配置 `checkout_paths` 时按分片工作副本逐项 checkout/update；未配置或配置为空时按完整项目根目录执行一次 checkout/update。
- 整项目初始化会直接校验 `repository_url`，并拒绝覆盖非空普通目录或混用已有分片 SVN 工作副本；已有根 `.svn` 时会核对 SVN URL 后再更新。
- 顶层 `username` 可供所有项目共用，项目级 `username` 仍可作为兼容性覆盖值。
- 修复 `data_sources.source_paths` 被误识别成整项目 `checkout_paths` 的问题。
- 增加整项目与分片项目的配置解析、工作副本识别和初始化模式测试。
